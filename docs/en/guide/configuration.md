...
> **Custom configuration**: A clean default config is built in (based on `config.js`, with no machine-specific paths). To override the defaults, choose one:
...
> - **Local run**: `cp config.js config.js` and edit it (`config.js` is git-ignored and won't be committed).
