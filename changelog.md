...
- **Subsonic 收藏 (红心) 状态全局联动修复**:
  ...
  - **红心状态全局透传**: 改进底层的 `musicToSongFlat` 等序列化映射，现在无论是搜索列表、专辑详情、流派浏览还是随机打乱等 11 个接口，只要歌曲存在于当前用户的收藏中，均能准确返回 `starred` 时间戳，完美点亮客户端的红心图标。
  - **收藏状态零延迟同步**: 修复了此前执行收藏/取消收藏动作时，服务端底层的 `loveIdSets` 未同步更新的 Bug。点按红心不仅立刻落盘，还会瞬间同步内存态，使客户端下次请求时状态不再滞后。
## v2.0.1 (2026-07-23)
