...
export const downloadAndCache = async (songInfo: any, url: string, quality?: string, username?: string, signal?: AbortSignal, isOnlyDownload?: boolean, shouldCacheLyric: boolean = true, shouldEmbedLyric: boolean = true, provenance: DownloadProvenance = {}) => {
    ...
    // 若调用方传入 policy（例如 subsonic 的"边播边存"），按策略判断是否需替换已有缓存
    // （音质升级 / 完整性修复等）；否则同音质已缓存则直接跳过下载
    let shouldSkip = result.exists && !result.isCollision
    if (shouldSkip && (provenance as any)?.policy) {
      const ev = evaluateCacheState({ ...songInfo, quality }, targetUsername, (provenance as any).policy)
      shouldSkip = !!(ev.exists && !ev.shouldReplace)
    }
    if (shouldSkip) {
