            ...
            if (result && result.url) {
                ...
                if ((global.lx.config as Record<string, any>)['subsonic.autoCacheOnPlay'] !== false) {
                  ...
                  setImmediate(() => {
                    ...
                    try {
                      // 不再依赖音乐库 PR 新增的 evaluateCacheState：downloadAndCache 自身已做存在性去重
                      // （与上游行为一致）。policy 交由 downloadAndCache 消费，用于音质升级/完整性替换，
                      // 音乐库 PR 合入并收编该逻辑后生效；未合入时退化为仅去重，不影响播放与边播边存。
                      const provenance: any = {
                        requestedSource: source,
                        downloadSource: fileCache.detectDownloadSource(result.url, source),
                        sourceName: (result as any).sourceName,
                        policy: { qualityUpgrade: true, integrity: true },
                      }
                      void fileCache.downloadAndCache(musicInfo, result.url, quality, cacheUsername, undefined, false, true, true, provenance)
                    ...
                    } catch (e) { console.warn('[Subsonic] auto-cache error:', e) }
                  })
                }
                if ((global.lx.config as Record<string, any>)['subsonic.enableDebug']) {
