...
RUN apk add --update \
  ...
  && npm install --ignore-scripts --no-audit --no-fund --registry=https://registry.npmmirror.com && npm run build \
  && rm -rf node_modules && npm install --omit=dev --no-audit --no-fund --registry=https://registry.npmmirror.com \
